...
app.get('/todos/', checkRequestsQueries, async (request, response) => {
  ...
  const getTodosQuery = `
    SELECT id,todo,priority,status,category,due_date AS dueDate FROM todo WHERE todo LIKE '%${search_q}%' AND priority LIKE '%${priority}%'
...
app.put('/todos/:todoId/', checkRequestBody, async (request, response) => {
  ...
  switch (true) {
    ...
    case category !== undefined:
      updateCategoryQuery = `UPDATE todo SET category='${category}' WHERE id=${todoId};`
